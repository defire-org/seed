...
contract pnDFIRE is ERC20, ERC20Burnable, Pausable, AccessControl {
  bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
  bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

  constructor() ERC20("pnDFIRE", "pnDFIRE") {
...
contract DeFIREGameOn is Ownable, ReentrancyGuard {
  ...
  // three tokens , three locking periods
  address public immutable pnDFIREAddress;
  ...
  mapping(address => mapping(uint256 => uint256)) public phasedPledgeNative;
  ...
  uint256 public minimumNative = 100;
  uint256 public minimumStable = 100;
  uint256 public phaseTime = 100;

  // ------------------------------------------------------------------------------- events

  event DepositedStable(uint256 indexed phase, address indexed wallet, uint256 amount);
  event DepositedNative(uint256 indexed phase, address indexed wallet, uint256 amount);
  ...
  constructor(
    ...
    address _pnDFIREAddress,
    address _psDFIREAddress,
    address _multisigAddress,
    uint256 _sessionStartTime,
    uint256 _phaseTime,
    uint256 _minimumStable,
    uint256 _minimumNative
  ) {
    require(_sessionStartTime > _phaseTime, "sessionStartTime must be greater than phase time");

    require(_stableAddress != address(0), "Address cannot be 0x");
    require(_xFLAREAddress != address(0), "Address cannot be 0x");
    require(_pnDFIREAddress != address(0), "Address cannot be 0x");
    require(_psDFIREAddress != address(0), "Address cannot be 0x");

    require(_minimumNative > 1, "Minimum deposit should be higher than 1");
  ...
  function amountDepositedNative() public view onlyOwner returns (uint256) {
  ...
  function myPNDFIRE() public view returns (uint256) {
    return investorMatrix[msg.sender].pnDFIRE;
  }
  ...
  function depositNative() external payable {
    require(msg.sender != multisigAddress, "MultiSig address cannot deposit");
    require(msg.value >= minimumNative, "Less than minimum deposit");
    require(seedPhase() > 0, "Seed Round Closed");

    if (seedPhase() < 3) {
      require(flareToken.balanceOf(msg.sender) >= 3 ether, "Not enough FLARE to deposit");
    }

    InvestorStats storage investor = investorMatrix[msg.sender];

    pnDFIREToken.mint(msg.sender, msg.value);

    investor.depositedNative += msg.value;
    investor.pnDFIRE += msg.value;

    phasedPledgeNative[msg.sender][seedPhase()] += msg.value;

    emit DepositedNative(seedPhase(), msg.sender, msg.value);
  }

  function depositStable(uint256 _amount, string memory _referral) external nonReentrant {
    require(msg.sender != multisigAddress, "MultiSig address cannot deposit");
    require(stableToken.balanceOf(msg.sender) >= _amount, "Not enough USDC to deposit");
